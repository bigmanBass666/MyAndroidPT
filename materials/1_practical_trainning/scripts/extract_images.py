...
for seq, (page, rel_id) in enumerate(unique, 1):
    ...
    if ext == '.emf':
        # EMF format - Pillow and most tools can't convert it
        # Save as-is with .emf extension since no converter available
        out_path = os.path.join(OUT_DIR, out_name)
        shutil.copy2(src_path, out_path)
        results.append((out_name, page, 'EMF', None, None))
        print(f"  [注意] {out_name} 为 EMF 格式，LibreOffice 未安装，无法转换。已保留原始文件。")
    else:
        out_path = os.path.join(OUT_DIR, out_name)
        shutil.copy2(src_path, out_path)
...
for name, page, fmt, w, h in results:
    if w and h:
        print(f"  {name}  页码~{page}  {fmt}  {w}x{h}px")
    else:
        print(f"  {name}  页码~{page}  {fmt}  (无法获取尺寸)")
